...
# Configuration
N8N_WEBHOOK_URL = os.getenv("N8N_WEBHOOK_URL", "https://arootah.app.n8n.cloud/webhook-test/alert_agent")
...
async def call_webhook(payload: Dict[str, Any]) -> Dict[str, Any]:
    """
    Call an n8n webhook with the given payload and wait for the response.
...
async def handle_message(ctx: Context, sender: str, msg: ChatMessage):
    ...
    if message_text:
        try:
            ...
            result = await call_webhook(payload)
            
            # Process the response from n8n
            response_text = result.get("response", "Received your message, but no specific response was provided.")
